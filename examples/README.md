...
| [`values-psa-restricted.yaml`](values-psa-restricted.yaml) | Run under the Kubernetes `restricted` Pod Security Standard: `runAsNonRoot` plus no privilege escalation, all capabilities dropped, `RuntimeDefault` seccomp and a read-only root filesystem. Leaves the chart's per-component `fsGroup: 0` defaults alone, since `restricted` places no constraint on `fsGroup`. See [Pod and container security contexts](../README.md#pod-and-container-security-contexts). |
| [`values-restricted-group-ranges.yaml`](values-restricted-group-ranges.yaml) | For the narrower case of a policy that constrains group IDs to a numeric range (Gatekeeper's `K8sPSPAllowedUsers`, an equivalent Kyverno policy, or an OpenShift SCC) and so rejects `fsGroup: 0`. Overrides `fsGroup` on the four components that ship it. Not required by PSA `restricted` — changing `fsGroup` on existing volumes triggers a recursive ownership change, so read the warning in the file first. |
